  ...
  return (
    <div className="screen-container pt-4">
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
        ...
        <h2 style={{ fontSize: '20px', fontWeight: 800 }}>Ledger Book</h2>
      </div>
        ...
        {loans.map((loan, idx) => (
          <div key={loan.id} className="party-row" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', color: 'var(--text-2)' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><BookOpen size={14}/> Principal: ₹{loan.loan_amount}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Clock size={14}/> Paid: ₹{loan.collected_amount}</span>
            </div>
          </div>
        ))}
        {loans.length === 0 && (
