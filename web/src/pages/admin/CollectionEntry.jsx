...
import { ArrowLeft, Wallet, Banknote, Search, ArrowUpDown, Filter } from 'lucide-react';
...
export default function CollectionEntry() {
  ...
  const [sortBy, setSortBy] = useState('balance'); // 'balance' | 'name' | 'newest'
  ...
  const processedLoans = useMemo(() => {
    let filtered = loans;
    if (searchQuery) {
      filtered = loans.filter(l => l.customer_name.toLowerCase().includes(searchQuery.toLowerCase()));
    }
        ...
        {loans.length === 0 ? (
          ...
          <>
            <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
              ...
              <button onClick={() => setShowFilters(!showFilters)} style={{ background: showFilters ? 'var(--brand-soft)' : 'var(--surface)', border: `1px solid ${showFilters ? 'var(--brand)' : 'var(--border)'}`, color: showFilters ? 'var(--brand)' : 'var(--text)', padding: '0 16px', borderRadius: '16px', cursor: 'pointer', transition: 'all 0.2s' }}>
                <Filter size={18} />
              </button>
            </div>
            ...
            {showFilters && (
              <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', overflowX: 'auto', paddingBottom: '4px' }}>
                {['balance', 'name', 'newest'].map(type => (
                  <button 
                    key={type}
                    onClick={() => setSortBy(type)}
                    style={{
                      padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: 600, border: 'none', cursor: 'pointer', whiteSpace: 'nowrap',
                      background: sortBy === type ? 'var(--text)' : 'var(--surface)',
                      color: sortBy === type ? 'var(--bg)' : 'var(--text-2)'
                    }}
                  >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px' }}>
              {processedLoans.map(loan => (
                ...
                >
                  <div className="party-avatar" style={{ transform: 'scale(1)' }}>{loan.customer_name.charAt(0).toUpperCase()}</div>
                  <div style={{ flex: 1 }}>
                    <h3 style={{ fontSize: '16px', fontWeight: 700 }}>{loan.customer_name}</h3>
                    <div className="text-muted" style={{ fontSize: '12px', marginTop: '2px' }}>Due Date: {loan.closing_date}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div className="text-muted" style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Balance</div>
                    <div className={loan.pending_amount > 0 ? "text-warning" : "text-green"} style={{ fontWeight: 800, fontSize: '15px' }}>₹{loan.pending_amount}</div>
                  </div>
                </div>
              ))}
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '12px', color: 'var(--text-2)' }}>Status: <span className="text-green" style={{textTransform: 'uppercase'}}>{selectedLoan.status}</span></div>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg)', borderRadius: '12px', padding: '16px', marginBottom: '20px' }}>
           ...
           <div style={{ height: '8px', background: 'var(--surface-3)', borderRadius: '4px', overflow: 'hidden' }}>
             <div style={{ width: `${clampedProgress}%`, height: '100%', background: 'var(--green)', transition: 'width 0.5s ease' }}></div>
           </div>
        </div>
        
        <div className="detail-grid">
          <div className="detail-row">
            <span className="detail-label">Principal Amount</span><span className="detail-value text-muted">₹{selectedLoan.loan_amount}</span>
          </div>
          <div className="detail-row">
            <span className="detail-label">Total End Due</span><span className="detail-value bg-surface">₹{selectedLoan.due_amount}</span>
          </div>
          <div className="detail-row">
            <span className="detail-label">Recovered</span><span className="detail-value text-green">₹{selectedLoan.collected_amount}</span>
          </div>
          <div className="detail-row pt-2" style={{ borderTop: '1px solid var(--border)', paddingTop: '12px' }}>
            <span className="detail-label text-warning" style={{ fontWeight: 700 }}>Outstanding Balance</span><span className="detail-value text-warning" style={{ fontSize: '18px' }}>₹{selectedLoan.pending_amount}</span>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: '24px' }}>
        ...
        <div style={{ position: 'relative', marginBottom: '20px' }}>
          ...
          <input 
            ...
            placeholder="0.00" 
            style={{
              width: '100%', background: 'var(--bg)', border: '2px solid var(--brand-soft)', borderRadius: '16px',
              padding: '20px 20px 20px 48px', fontSize: '32px', fontWeight: 800, color: 'var(--text)', outline: 'none', transition: 'border-color 0.2s'
            }}
          />
        </div>
